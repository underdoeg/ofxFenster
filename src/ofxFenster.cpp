...
//ofBaseApp *	ofxFenster::ofAppPtr;
//ofxFenster	* ofxFenster::instance;
//GLFWwindow* ofxFenster::windowP = NULL;
...
//------------------------------------------------------------
void ofxFenster::setMultiDisplayFullscreen(bool bMultiFullscreen){
    bMultiWindowFullscreen = bMultiFullscreen; 
}

//------------------------------------------------------------
void ofxFenster::setDoubleBuffering(bool doubleBuff){
	bDoubleBuffered = doubleBuff;
}

//------------------------------------------------------------
void ofxFenster::setColorBits(int r, int g, int b){
	rBits=r;
	gBits=g;
	bBits=b;
}
...
//------------------------------------------------------------
void ofxFenster::setAlphaBits(int a){
	aBits=a;
}

//------------------------------------------------------------
void ofxFenster::setDepthBits(int depth){
	depthBits=depth;
}

//------------------------------------------------------------
void ofxFenster::setStencilBits(int stencil){
	stencilBits=stencil;
}
...
//------------------------------------------------------------
void ofxFenster::setOpenGLVersion(int major, int minor){
...
void ofxFenster::setupOpenGL(int w, int h, int screenMode){
	...
	if(!glfwInit( )){
		ofLogError("ofxFenster") << "couldn't init GLFW";
		return;
	}
...
#ifdef TARGET_LINUX
//------------------------------------------------------------
void ofxFenster::setWindowIcon(const string & path){
    ofPixels iconPixels;
...
//------------------------------------------------------------
void ofxFenster::setWindowIcon(const ofPixels & iconPixels){
	iconSet = true;
...
//--------------------------------------------
void ofxFenster::runAppViaInfiniteLoop(ofBaseApp * appPtr){
	glfwMakeContextCurrent(windowP);
	
	ofxFensterManager::get()->runAppViaInfiniteLoop(appPtr);
}
...
int ofxFenster::getCurrentMonitor(){
...
//------------------------------------------------------------
void ofxFenster::exitApp(){
	ofLogVerbose("ofxFenster") << "terminating GLFW app!";
...
//------------------------------------------------------------
void ofxFenster::listVideoModes(){
	int numModes;
...
//------------------------------------------------------------
bool ofxFenster::isWindowIconified(){
...
//------------------------------------------------------------
void ofxFenster::iconify(bool bIconify){
	if(bIconify)
...
Window ofxFenster::getX11Window(){
	return glfwGetX11Window(windowP);
}
#endif

#if defined(TARGET_LINUX) && !defined(TARGET_OPENGLES)
GLXContext ofxFenster::getGLXContext(){
	return glfwGetGLXContext(windowP);
}
#endif

#if defined(TARGET_LINUX) && defined(TARGET_OPENGLES)
EGLDisplay ofxFenster::getEGLDisplay(){
	return glfwGetEGLDisplay();
}

EGLContext ofxFenster::getEGLContext(){
	return glfwGetEGLContext(windowP);
}

EGLSurface ofxFenster::getEGLSurface(){
	return glfwGetEGLSurface(windowP);
}
#endif

#if defined(TARGET_OSX)
void * ofxFenster::getNSGLContext(){
	return glfwGetNSGLContext(windowP);
}

void * ofxFenster::getCocoaWindow(){
	return glfwGetCocoaWindow(windowP);
}
#endif

#if defined(TARGET_WIN32)
HGLRC ofxFenster::getWGLContext(){
	return glfwGetWGLContext(windowP);
}

HWND ofxFenster::getWin32Window(){
